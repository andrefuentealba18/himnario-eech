...
export default function HomePage() {
  return (
    <div className="relative min-h-screen w-full bg-background text-foreground overflow-x-hidden font-body transition-colors duration-500">
      ...
      <div className="fixed inset-0 -z-20 overflow-hidden">
        ...
        <div className="absolute top-[-10%] -left-[10%] w-[70%] h-[70%] bg-blue-400/10 dark:bg-blue-600/5 rounded-full filter blur-[120px] animate-pulse-slow" />
        <div className="absolute bottom-[-10%] -right-[10%] w-[80%] h-[80%] bg-primary/10 dark:bg-primary/5 rounded-full filter blur-[120px] animate-pulse-slow" style={{ animationDelay: '2s' }} />
        <div className="absolute top-[20%] right-[-5%] w-[45%] h-[45%] bg-indigo-400/10 dark:bg-indigo-600/5 rounded-full filter blur-[100px] animate-pulse-slow" style={{ animationDelay: '4s' }} />
        <div className="absolute bottom-[15%] left-[5%] w-[50%] h-[50%] bg-violet-400/10 dark:bg-violet-600/5 rounded-full filter blur-[110px] animate-pulse-slow" style={{ animationDelay: '1s' }} />
      </div>

      <div className="relative z-10 flex flex-col min-h-screen">
        {/* Botón de Modo Oscuro Superior Derecho */}
        <div className="absolute top-4 right-4 md:top-6 md:right-6 animate-in fade-in zoom-in duration-700">
          <ThemeToggle />
        </div>
        ...
        <main className="container max-w-2xl mx-auto flex-1 flex flex-col items-center justify-center p-6">
          ...
          <header className="w-full text-center space-y-2 mb-8 animate-in fade-in slide-in-from-top-4 duration-700">
            <div className="inline-flex items-center justify-center p-4 bg-card rounded-3xl mb-4 shadow-sm border border-slate-100 dark:border-slate-800 animate-float">
            ...
            {navigationItems.map((item) => (
              <div key={item.title} className="animate-in fade-in zoom-in-95 duration-500" style={{ animationDelay: item.delay }}>
                <Link href={item.href} className="group block h-full">
                  <Card className="h-full border-slate-200/60 dark:border-slate-800/60 bg-card/80 backdrop-blur-sm hover:bg-card transition-all duration-300 hover:shadow-md hover:-translate-y-1 active:scale-95 overflow-hidden relative shadow-sm border-2">
                    <CardContent className="flex flex-col items-center justify-center p-6 text-center h-full min-h-[140px]">
                      ...
                      <h3 className="font-bold text-[10px] md:text-xs uppercase tracking-wider font-body text-foreground group-hover:text-primary transition-colors leading-tight px-2">
                        {item.title}
                      </h3>
                    </CardContent>
                  </Card>
          ...
          <footer className="w-full text-center mt-12 pb-10 space-y-6 animate-in fade-in duration-1000 delay-500">
            <div className="h-px w-16 bg-slate-300/50 dark:bg-slate-700/50 mx-auto" />
